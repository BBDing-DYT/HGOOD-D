...
from utils.k_means import K_Means_hyper
...
def hierarchical_clustering_K_Means(option, x, num_cluster, hyper_c=0.):
    ...
    # number of levels
    level = len(num_cluster)
    data_queue = [x.detach().cpu().numpy().astype('float64')]
    results = {'im2cluster': [], 'centroids': [], 'density': []}
    for i in range(level):
        # perform k means for every level
        initial_centers = kmeans_plusplus_initializer(data_queue[len(data_queue) - 1], num_cluster[i]).initialize()
        if hyper_c == 0.:
            k_means = K_Means_hyper(data_queue[len(data_queue) - 1], initial_centers, ccore=False, itermax=130)
            k_means.process()
        else:
            hyper_metric = distance_metric(type_metric.USER_DEFINED,
                                           func=lambda x, y: _dist_matrix_pyclustering(x, y, hyper_c))
            k_means = K_Means_hyper(data_queue[len(data_queue) - 1], initial_centers, ccore=False, itermax=100,
