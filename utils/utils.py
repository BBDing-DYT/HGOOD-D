...
def visualize_hierarchical_clustering_with_labels(x, centroids, labels, method='tsne', name='None'):
    ...
    # 合并 x 和 centroids 进行降维
    combined = np.vstack((x, *centroids))
    combined_reduced = reducer.fit_transform(combined)

    # 分离降维后的 x 和 centroids
    x_reduced = combined_reduced[:x.shape[0], :]
    centroids_reduced = [combined_reduced[x.shape[0] + sum(len(c) for c in centroids[:i]):x.shape[0] + sum(len(c) for c in centroids[:i+1]), :] for i in range(len(centroids))]

    # 绘制降维后的节点
    plt.figure(figsize=(10, 8))
    scatter = plt.scatter(x_reduced[:, 0], x_reduced[:, 1], c=labels, cmap='coolwarm', s=5, alpha=0.7)
    plt.colorbar(scatter, ticks=[0, 1])
    for i, c_reduced in enumerate(centroids_reduced):
        plt.scatter(c_reduced[:, 0], c_reduced[:, 1], label=f'centroids level {i+1}', s=50, alpha=0.7)
    plt.legend()
